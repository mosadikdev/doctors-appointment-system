  ...
  const [savedDoctors, setSavedDoctors] = useState([]);
    ...
    async function fetchData() {
      try {
        ...
        if (!upcomingRes.ok) throw new Error("Appointments request failed");
        const upcomingData = await upcomingRes.json();
        setUpcomingAppointments(upcomingData);

        const savedRes = await fetch(`${baseURL}/api/patient/saved-doctors`, { headers });
        if (!savedRes.ok) throw new Error("Doctors request failed");
          ...
          {upcomingAppointments.length === 0 ? (
            ...
            upcomingAppointments.map((appointment) => (
              ...
              >
                ...
                <button className="mt-3 sm:mt-0 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                  View Details
                </button>
              </div>
            ))
                          ...
                          {[...Array(5)].map((_, i) => (
                            <svg
                              ...
                              className={`w-4 h-4 fill-current ${
                                i < Math.round(doctor.rating) ? "" : "text-gray-300"
                              }`}
                              viewBox="0 0 24 24"
                            >
